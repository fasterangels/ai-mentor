"""
Sample recorded platform v2: recorded fixtures only, no live IO.
Loads JSON from backend/ingestion/fixtures/sample_recorded_platform_v2/.
Deterministic mapping; no HTTP or network. Missing required fields raise ValueError.
"""
...
class SampleRecordedPlatformV2Adapter(RecordedPlatformAdapter):
    """Recorded-only adapter. No live IO; all data from local fixture files."""

    def __init__(self, fixtures_dir: Path | None = None) -> None:
        if fixtures_dir is None:
            base = Path(__file__).resolve().parent.parent.parent
            fixtures_dir = base / "ingestion" / "fixtures" / "sample_recorded_platform_v2"
        self._fixtures_dir = Path(fixtures_dir)

    @property
    def name(self) -> str:
        return "sample_recorded_platform_v2"
