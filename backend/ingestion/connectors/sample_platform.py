"""
Sample platform adapter: recorded fixtures only, no HTTP.
Loads JSON from backend/ingestion/fixtures/sample_platform/.
Deterministic mapping; missing required fields raise ValueError.
"""
...
class SamplePlatformAdapter(RecordedPlatformAdapter):
    """Adapter that reads fixtures from ingestion/fixtures/sample_platform/*.json."""

    def __init__(self, fixtures_dir: Path | None = None) -> None:
        if fixtures_dir is None:
            # backend/ingestion/connectors/sample_platform.py -> backend/ingestion/fixtures/sample_platform
            base = Path(__file__).resolve().parent.parent.parent
            fixtures_dir = base / "ingestion" / "fixtures" / "sample_platform"
    ...
    def load_fixtures(self) -> List[Dict[str, Any]]:
        """Load all JSON fixture files from the fixtures directory (deterministic order)."""
