...
def compute_would_refuse(profile: UncertaintyProfile) -> bool:
    """
    Whether this profile would lead to a refusal (any signal triggered).
    Measurement-only; no enforcement. Deterministic.
    """
    return any(s.triggered for s in profile.signals)
