...
class DecayModelParams:
    ...
    def penalty_for(self, age_band: str) -> float:
        """Return penalty factor for the given age_band (0..1). If band not found, return 1.0."""
        bands = self.bands or []
        penalty_by_band = self.penalty_by_band or []
        for i, b in enumerate(bands):
            if b == age_band and i < len(penalty_by_band):
                return float(penalty_by_band[i])
        return 1.0
