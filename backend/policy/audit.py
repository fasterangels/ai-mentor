"""
Decision audit: compare analyzer output under current vs proposed policy per snapshot.
Produces per-market rows with pick/confidence/reasons diff and deterministic change_reason.
"""
...
# Analyzer market id -> policy market key (audit output)
...
class AuditRow:
    ...
    changed: bool
    change_reason: str
    reasons_added: list[str] = field(default_factory=list)
    reasons_removed: list[str] = field(default_factory=list)


def _pick_from_decision(d: dict[str, Any]) -> str:
    """Single string for pick: selection if PLAY, else decision kind."""
...
def _reasons_from_decision(d: dict[str, Any]) -> list[str]:
...
def _infer_change_reason(
    ...
    """Deterministic short string explaining why the decision changed."""
    if before_pick == after_pick:
        return ""
    # Stricter policy (higher min_confidence): PLAY/NO_BET can become NO_BET/NO_PREDICTION due to threshold
    if min_proposed > min_current:
        if before_pick == "PLAY" and after_pick in ("NO_BET", "NO_PREDICTION"):
            if before_conf is not None and min_proposed > before_conf:
                return "min_confidence_gate"
            return "min_confidence_gate"
        if before_pick == "NO_BET" and after_pick == "NO_PREDICTION":
            return "min_confidence_gate"
    if min_proposed < min_current:
        if after_pick == "PLAY" and before_pick in ("NO_BET", "NO_PREDICTION"):
...
def audit_snapshots(
    ...
    """
    Run analysis twice per snapshot (current vs proposed); compare per-market.
    Returns AuditReport dict with rows, summary counts, and checksums.
    """
    ...
    for item in snapshots:
        ...
        for market_id in MARKETS_V2:
            ...
            before_conf = d_before.get("confidence") if d_before else None
            if before_conf is not None:
                before_conf = float(before_conf)
            after_conf = d_after.get("confidence") if d_after else None
            ...
            change_reason = _infer_change_reason(
                before_pick, after_pick, before_conf, after_conf, min_current, min_proposed
            ) if changed else ""
    ...
    unchanged_count = total_markets - changed_count
    per_market_changes: dict[str, int] = {}
    for r in rows:
        per_market_changes[r.market] = per_market_changes.get(r.market, 0) + (1 if r.changed else 0)

    # Checksums
    snapshots_checksum = checksum_report({
        "snapshots_count": len(snapshots),
        "match_ids": [s.get("match_id") for s in snapshots],
    })
    current_policy_checksum = checksum_report(current_policy.model_dump(mode="json"))
    proposed_policy_checksum = checksum_report(proposed_policy.model_dump(mode="json"))
    ...
    return {
        "summary": {
            ...
            "per_market_change_count": per_market_changes,
        },
        "rows": [
            {
                "snapshot_id": r.snapshot_id,
                "market": r.market,
                "before_pick": r.before_pick,
                "after_pick": r.after_pick,
                "before_confidence": r.before_confidence,
                "after_confidence": r.after_confidence,
                "changed": r.changed,
                "change_reason": r.change_reason,
                "reasons_added": r.reasons_added,
                "reasons_removed": r.reasons_removed,
            }
            for r in rows
        ],
        "snapshots_checksum": snapshots_checksum,
        "current_policy_checksum": current_policy_checksum,
        "proposed_policy_checksum": proposed_policy_checksum,
    }
